...
class ConversationViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if profile_report_ids is not None:
            profile_reports = ProfileReport.objects.filter(id__in=profile_report_ids.split(","))
            queryset = self.filter_queryset(self.get_queryset()).filter(profile_report__in=profile_reports)

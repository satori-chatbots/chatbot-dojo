...
class Conversation(models.Model):
    ...
    test_report = models.ForeignKey(
        TestReport, related_name="conversations", on_delete=models.CASCADE
    )
    ...
    context_details = (
        models.JSONField()
    )  # Store additional context like "you are vegan"

    # Ask About & Parameters
    ask_about = models.JSONField()  # Store list of questions/topics
    parameters = models.JSONField()  # Store drink, number, size, pizza_type etc

    # Conversation Stats
    interaction_style = models.CharField(max_length=50)
    steps = models.IntegerField()
    all_answered = models.BooleanField(default=False)

    # Output Data
    data_output = models.JSONField()  # Store price, time, order_id etc

    # Errors & Performance
    errors = models.JSONField()  # Store error codes and counts
    total_cost = models.DecimalField(max_digits=10, decimal_places=7)
    conversation_time = models.FloatField()

    # Response Times
    response_times = models.JSONField()  # Store list of response times
    response_time_avg = models.FloatField()
    response_time_max = models.FloatField()
    response_time_min = models.FloatField()

    # Interaction History
    interaction_history = models.JSONField()  # Store user-assistant conversation pairs

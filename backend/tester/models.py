...
class TestFile(models.Model):
    """
    Model to store the uploaded YAML files
...
class TestError(models.Model):
    """
    Test Error is a model to store the errors in a Test Report, it contains the error code and the number of times it has occurred and the relative path of the conversation files with that error
    """
...
class TestReport(models.Model):
    ...
    # Test report contains one or more errors but an error is only related to one test report
    errors = models.ForeignKey(
        TestError, related_name="test_reports", on_delete=models.CASCADE
    )
...
class TestCase(models.Model):
    ...
    # Test case contains one or more reports but a report is only related to one test case
    reports = models.ForeignKey(
        TestReport, related_name="test_cases", on_delete=models.CASCADE
    )

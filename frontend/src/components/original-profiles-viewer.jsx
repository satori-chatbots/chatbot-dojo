...
import { Users, ArrowLeft, AlertCircle, FileText, Copy, Check } from "lucide-react";
...
const YamlHighlighter = ({ content }) => {
  const highlightYaml = (yaml) => {
    let highlighted = yaml
      // Comments
      .replace(/(#.*$)/gm, '<span class="text-success-600 dark:text-success-400">$1</span>')
      // Keys (before colon)
      .replace(/^(\s*)([a-zA-Z_][a-zA-Z0-9_-]*)\s*:/gm, '$1<span class="text-primary-600 dark:text-primary-400 font-semibold">$2</span>:')
      // String values (quoted)
      .replace(/:\s*["']([^"']*?)["']/g, ': <span class="text-warning-600 dark:text-warning-400">"$1"</span>')
      // Numbers
      .replace(/:\s*(\d+\.?\d*)\s*$/gm, ': <span class="text-secondary-600 dark:text-secondary-400">$1</span>')
      // Booleans
      .replace(/:\s*(true|false)\s*$/gm, ': <span class="text-danger-600 dark:text-danger-400">$1</span>')
      // Array items
      .replace(/^(\s*)-\s+/gm, '$1<span class="text-default-600 dark:text-default-400">-</span> ');
      ...
      <ModalHeader className="flex flex-col gap-1">
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold">Original TRACER Profiles</h2>
              <p className="text-sm text-default-500">
                {execution.execution_name} - {profilesData?.project_name || execution.project_name}
              </p>
            </div>
          </div>
                ...
                <Card className="border-success-200 bg-success-50 dark:bg-success-900/20">
                  <CardBody className="text-center py-4">
                    <p className="text-success-700 dark:text-success-400">
                      <strong>{profilesData.profiles.length}</strong> original profiles found.
                      These are read-only copies of the profiles as originally generated by TRACER.
                    </p>
                  </CardBody>
                </Card>
                      ...
                      <div className="px-4 pb-4">
                        <div className="flex justify-between items-center mb-3">
                          <p className="text-sm text-default-600">
                            Original TRACER-generated profile content (read-only)
                          </p>
                          <Button
                            size="sm"
                            variant="light"
                            ...
                            onPress={async () => {
                              try {
                                await navigator.clipboard.writeText(profile.content);
                                showToast(`${profile.filename} copied to clipboard`, "success");
                              } catch (err) {

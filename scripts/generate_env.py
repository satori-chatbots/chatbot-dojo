...
def _load_existing_env(env_file: Path) -> dict:
    """Load and clean existing env file content."""
    env_content = {}
    if env_file.exists():
...
def _ensure_env_keys(env_content: dict) -> bool:
    """Ensure required keys exist in env_content, generating them if needed."""
...
def update_env_file(backend_dir: Path) -> bool:
    """Create or update the .env file with necessary keys, quoting the values."""
    env_file = backend_dir / ".env"
    env_content = _load_existing_env(env_file)
    keys_generated = _ensure_env_keys(env_content)
